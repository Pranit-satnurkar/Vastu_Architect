...
Exterior means "not backed by another room" — so a wall facing the garden
cut-out is exterior too. Purely geometric — no compass/Vastu preference.
"""
...
from .geometry import Rect, exterior_walls
...
def place_windows(rooms: List[Rect], rng: random.Random) -> None:
    for r in rooms:
        spec = r.meta.get("spec")
        if spec is not None and not spec.needs_window:
            continue
        ext = exterior_walls(r, rooms)
        if not ext:
            continue
        ...
        pos = round(rng.uniform(0.4, 0.6), 2)
